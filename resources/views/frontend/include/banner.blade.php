                ...
                <div class="w3ls-header-right">
                    <ul>
                        ...
                        @if(\Illuminate\Support\Facades\Session::get('customer_id'))
                            <li class="head-dpdn">
                                <a href="" onclick="event.preventDefault(); document.getElementById('customerLogout').submit();"><i
                                        class="fa fa-sign-in" aria-hidden="true"></i> Logout</a>
                                <form id="customerLogout" action="{{route('customer-logout')}}" method="POST" class="d-none">
                                    @csrf
                                </form>
                            </li>
                        @else
                            <li class="head-dpdn">
                                <a href="{{route('form-login')}}"><i class="fa fa-sign-in" aria-hidden="true"></i> Login</a>
                            </li>
                        @endif
                        @if(\Illuminate\Support\Facades\Session::get('customer_id'))
                            <li class="head-dpdn">
                                <a href="#"><i class="fa fa-user-plus"
                                               aria-hidden="true"></i>{{\Illuminate\Support\Facades\Session::get('customer_name')}}
                                </a>
                            </li>
                        @else
                            <li class="head-dpdn">
                                <a href="{{route('customer-register')}}"><i class="fa fa-user-plus"
                                                                            aria-hidden="true"></i>
                                    Signup</a>
                            </li>
                        @endif
            ...
            <div class="container">
                <nav class="navbar navbar-default">
                    ...
                    <div class="collapse navbar-collapse" id="bs-megadropdown-tabs">
                        <ul class="nav navbar-nav navbar-right">
                            @foreach($categories as $cate)
                                <li><a href="{{route('category_dish',$cate->id)}}">{{$cate->category_name}}</a></li>
                            @endforeach
                    ...
                    <div class="cart cart box_1">
                        <a href="{{route('cart.index')}}" class="last">
